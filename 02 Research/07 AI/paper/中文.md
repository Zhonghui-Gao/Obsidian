...
为了证明改进模型的检测性能提升效果，我们进行了改进模型与基线模型YOLOv8s的对比实验。表1显示了改进模型和 YOLOv8 的每个类别的 AP 值和所有类别的mAP0.5值。如表6的比较结果所示，改进模型的mAP值提高了7.7%。所有类别的AP值都有不同程度的提升，hand-raising、writing、reading3个类别的AP值提升了10%以上。这表明改进后的模型可以有效提高小物体的检测精度，提高检测性能