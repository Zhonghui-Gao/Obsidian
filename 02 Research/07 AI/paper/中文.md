...
为了证明改进模型的检测性能提升效果，我们进行了改进模型与基线模型YOLOv8s的对比实验。表1显示了改进模型和 YOLOv8 的每个类别的 AP 值和所有类别的mAP0.5值。如表1和表2的比较结果所示，改进模型的mAP值几乎保持不变。但是在准确率上相比原模型提高1.5%，同时表明改进后的模型可以有效提高教室环境下学生行为的检测精度，提高检测性能。