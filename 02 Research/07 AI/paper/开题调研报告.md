...
Pabba等人[18]提出了一个新颖的智能视觉系统，用于在实体智能课堂中通过整合上半身姿势和面部特征自动监控个体学生和整个班级行为。创建了一个课堂自发学生行为数据集（CSSBD），其中包括展示真实课堂中观察到的七种不同学生行为的学生面部和上半身部分的图像。通过在CSSBD数据集上训练深度神经网络架构，开发了基于多种视觉特征的行为分类模型，其训练和验证准确率分别为91.30%和90.80%。

Zheng[25]等人设计实现了一个基于多维注意力机制的模型GestureDet用来检测并识别学生睡觉、举手和站立三种行为，并证明了注意力机制在学生行为识别方面的有效性。

Zheng R, Jiang F, Shen R. GestureDet: Real-time Student Gesture Analysis with
 Multi-dimensional Attention-based Detector[C]. Proceedings of the Twenty-Ninth
 International Conference on International Joint Conferences on Artificial Intelligence.
 2021: 680-686.


Sharma等人基于笔记本电脑内置的摄像头通过眼动追踪和面部表情分析，采用Haarn级联算法检测面部和眼睛区域结合卷积神经网络(CNN)进行注意力状态和面部表情分类。
...
Park等收集了韩国公立本科大学14 名在职前教师的面部表情，并使用情绪识别软件分析了这些面部表情所表达的情绪[25]。Kim等提出了一种基于情绪识别算法的智能课堂平台，以分析教师的情绪并量化教学过程中的各种参数[26]。Estrada 等使用情绪识别模型和进化方法来分析教师语料库中关于学生学习状态的情绪积极性[27]。Cabada等使用基于深度学习的情绪分析算法来检测学生和教师的课堂情绪[28]。Gutierrez等收集学生对教师教学行为的评论作为文本数据，通过机器学习、随机森林等算法对文本数据进行挖掘，以此完成情感分析[30]。

[25]S PARK, R JYU. “Exploring Preservice Teachers' Emotional Experiences in an Immersive Virtual Teaching Simulation through Facial Expression Recognition”,International Journal of Human-Computer Interaction, (6):521–33. 
[26]Y KIM TTOYOTA (2018) R BEHNAGH. “Towards Emotionally Aware AI Smart Classroom: Current Issues and Directions for Engineering and Education”,IEEE Access, (1):5308–31. 
[27] M L B ESTRADA, R Z CABADA, R O BUSTILLOS et al (2020) “Opinion mining and emotion recognition applied to learning environments”[J].Expert Syst Appl, (7):12–23. 
[28]R Z CABADA, M L B ESTRADA, R O BUSTILLOS (2018) “Mining of Educational Opinions with Deep Learning”[J].J Univers Comput Sci,(11):160426 
[30] G, GUTIERREZ, E J CANUL,O ZEZZATI A et al “Mining: Students Comments about Teacher Performance Assessment using Machine Learning Algorithms”, 2018(3). 
