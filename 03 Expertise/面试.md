### 1.自我介绍（中英）
大家好，我叫高忠辉，目前就读于东莞理工学院电子信息专业，研三在读。研究生期间，我连续两年获得校级三等奖学金，并取得了中级嵌入式软件设计师证书。同时，我以第一作者身份在C类会议发表论文一篇，以第二作者身份在EI会议发表论文一篇。
除了科研成果，我还积累了丰富的实习经验。第一段实习是在⽲⻨科技开发（深圳）有限公司，主要负责Rockchip平台的kernel与uboot移植，以及Qt相关开发工作。第二段实习是在科大讯飞，参与自研芯片的测试与开发，并积极参与开源项目的提交。

Hello everyone, my name is Gao Zhonghui. I am currently a third-year graduate student majoring in Electronic Information at Dongguan University of Technology. During my studies, I was awarded the university’s third-class scholarship for two consecutive years and obtained the Intermediate Embedded Software Designer certification. 
In addition, I have published two academic papers: one as the first author at a C-class conference, and another as the second author at an EI-indexed conference.
Beyond research, I gained valuable industry experience through two internships. At Wheat Technology Roymark Development (Shenzhen) Co., Ltd., I worked on Rockchip-related kernel and u-boot porting, as well as Qt development. Later, at iFLYTEK, I contributed to testing and development of self-developed chips and made submissions to open-source projects.
...
/**
 * @file
 * @brief 验证 Zephyr 中 DMA 使用 scatter‑gather 模式进行内存到内存的循环传输
 *
 * @details
 * 测试步骤：
 *   1. 配置 DMA，使能 scatter‑gather 功能。
 *   2. 将传输方向设置为内存到内存，并使用两个传输块描述符。
 *   3. 启动 DMA 传输，从 TX 缓冲区传输到 RX 缓冲区。
 *
 * 预期结果：
 *   - 数据应能够在无软件干预的情况下，从源缓冲区正确传输到目标缓冲区。
 */
...
static int global = 100; //每次都会初始化

void count_function(){
	static int count = 10;//初始化一次 下一个继续叠加 保留记忆
	count++;
}
...
14 Union 和struct 区别
联合体成员共用一片内存，结构体是所有的变量内存的叠加，需要考虑字节对齐问题，联合体来说，只要修改成员的数据就会修改其他成员的数据，结构体是不影响的。
联合体可以判断大小端问题：
大端字节：高字节放在低位 低字节放在高位
小端字节：高放高，低放低
```C
union my{
	short t;
...
int main(int argc, char *argv)
{
	MY test;
	test.b = 0x0102;
	if(test.b[0] == 0x10 && test.b[1] == 0x02)
		printf("大端字节=> %x, %x\n", test.b[0], test.b[1]);
	else if(test.b[0] == 0x02 && test.b[1] == 0x10)
		printf("小端字节=> %x, %x\n", test.b[0], test.b[1]);
	return 0;
}
```
...
15计算占用空间大小问题：
1.所占空间必须是成员变量字节最大的整数倍
2.每个变量类型的偏移量必须是该变量类型的整数倍
3.联合体中所有的变量都是共用一块内存，还需注意数组占用的最大内存
```C
Typedef union {doule I; int k[5], char c;} DATE;
```
联合体中成员变量最大的是`double` 8字节，所以最终肯定是8的整数倍，联合体是共占内存，`int` 4 * 5=20 字节， 所以是24字节
```C
...
double dog // 8字节
```
DATE 24字节 最大是8 所以是8的倍数 
![[Pasted image 20260113112953.png]]
...
uboot到内核启动 
uboot 传的是R0,R1,R2 参数，R0默认是0，R1存放的就是CPU的型号，R2里面存放的是些基地址，比如内存的起始地址，内存大小，根文件系统的信息 tagged_list的形式来传递参数
